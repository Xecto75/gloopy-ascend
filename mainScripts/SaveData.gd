...
func update_difficulty(level: int) -> void:
	...
	match level%10:
		1, 6:
			PLANET_SIZE_MIN = max(
				((MIN_PLANET_SIZE-MIN_PLANET_SIZE_START)/MAX_LEVEL)*level 
				+ MIN_PLANET_SIZE_START,
				MIN_PLANET_SIZE
			)
			PLANET_SIZE_MAX = max(
				((MIN_PLANET_SIZE-MAX_PLANET_SIZE_START)/MAX_LEVEL)*level 
				+ MAX_PLANET_SIZE_START,
				MIN_PLANET_SIZE
			)
		2, 7:
			PLANET_DISTANCE_MIN = min(
				((MIN_PLANET_DISTANCE_END-MIN_PLANET_DISTANCE_START)/MAX_LEVEL)*level 
				+ MIN_PLANET_DISTANCE_START,
				MIN_PLANET_DISTANCE_END
			)
			PLANET_DISTANCE_MAX = min(
				((MAX_PLANET_DISTANCE_END-MAX_PLANET_DISTANCE_START)/MAX_LEVEL)*level 
				+ MAX_PLANET_DISTANCE_START,
				MAX_PLANET_DISTANCE_END
			)
		3, 8:
			PLANET_POPULATION = max(
				1-((1-MIN_PLANET_POPULATION)/MAX_LEVEL)*level,
				MIN_PLANET_POPULATION
			)
		4, 9:
			FIREBALL_FREQUENCY = min(
				0.05+((1-MIN_FIREBALL_FREQUENCY)/MAX_LEVEL)*level,
				1
			)
		0, 5:
			LAVA_SPEED = min(
				((MAX_LAVA_SPEED-MIN_LAVA_SPEED)/MAX_LEVEL)*level 
				+ MIN_LAVA_SPEED,
				MAX_LAVA_SPEED
			)
		_:
